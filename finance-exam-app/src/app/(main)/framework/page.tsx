...
export default function FrameworkPage() {
  ...
  useEffect(() => {
    ...
    setChapters(framework as unknown as ChapterNode[]);
    setLoading(false);
  }, []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-text-primary">知识框架</h2>
        {chapters.length > 0 && (
          <span className="text-sm text-text-secondary">共 {chapters.length} 章</span>
        )}
      </div>

      <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
        <TreeView chapters={chapters} onSelectKp={setSelectedKp} />
        <div className="hidden lg:block">
          {selectedKp ? (
            <KnowledgePointDetail kp={selectedKp} onClose={() => setSelectedKp(null)} />
          ) : (
            <div className="rounded-xl bg-white p-6 text-center shadow-sm">
              <p className="text-sm text-text-secondary">点击左侧知识点查看详情</p>
            </div>
          )}
        </div>
      </div>
