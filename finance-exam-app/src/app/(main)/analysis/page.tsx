...
export default function AnalysisPage() {
  const [data, setData] = useState<{
    ...
    weakPoints: number;
    chapterBreakdown: Array<{ chapterId: number; title: string; totalKnowledgePoints: number; masteredKnowledgePoints: number }>;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    let weak = 0;
    let totalKp = 0;
    const chapterBreakdown: Array<{ chapterId: number; title: string; totalKnowledgePoints: number; masteredKnowledgePoints: number }> = [];
    ...
    for (const ch of framework) {
      ...
      chapterBreakdown.push({
        chapterId: ch.id,
        title: ch.title,
        totalKnowledgePoints: chKpCount,
        masteredKnowledgePoints: chMastered,
      });
    }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl bg-white p-8 text-center shadow-sm">
          ...
          <p className="text-text-secondary">完成一定量的题目后，系统将自动分析你的知识掌握情况</p>
          <p className="mt-1 text-sm text-text-secondary">去「刷题练习」开始做题吧</p>
        </div>
      </div>
    );
  }
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-4">
          <div className="flex h-4 rounded-full bg-page-bg overflow-hidden">
            <div className="bg-success transition-all" style={{ width: `${data.totalKnowledgePoints > 0 ? (data.masteredKnowledgePoints / data.totalKnowledgePoints) * 100 : 0}%` }} />
            <div className="bg-warning transition-all" style={{ width: `${data.totalKnowledgePoints > 0 ? (data.weakPoints / data.totalKnowledgePoints) * 100 : 0}%` }} />
            <div className="bg-primary-lighter flex-1" />
          </div>
          <div className="mt-3 flex justify-between text-sm text-text-secondary">
            <span>已掌握: {data.totalKnowledgePoints > 0 ? Math.round((data.masteredKnowledgePoints / data.totalKnowledgePoints) * 100) : 0}%</span>
            ...
            {data.chapterBreakdown.map((ch) => (
              <div key={ch.chapterId}>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-text-secondary">{ch.masteredKnowledgePoints}/{ch.totalKnowledgePoints} 个知识点</span>
                </div>
                <div className="mt-1 h-2 rounded-full bg-page-bg">
                  <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${ch.totalKnowledgePoints > 0 ? (ch.masteredKnowledgePoints / ch.totalKnowledgePoints) * 100 : 0}%` }} />
                </div>
              </div>
            ))}
          </div>
